...
const createClient = (id) => {
    ...
    // update hanya untuk mode gambar
    // Map untuk menyimpan caption terakhir per user
    ...
    //         // Update caption terakhir kalau ada teks
    //         if (msg.body && msg.body.trim() !== '') {
    //             activeCaptions[msg.from] = msg.body;
    //         }

    //         // Kalau pesan ada media
    //         if (msg.hasMedia) {
    //             const media = await msg.downloadMedia();
    //             const ext = media.mimetype.split('/')[1];

    //             // Gunakan caption dari msg.body atau fallback ke caption terakhir
    //             let rootFolder = msg.body
    //                 ? msg.body.split('/').map(s => s.replace(/[^a-zA-Z0-9-_]/g, '_')).join('/')
    //                 : (activeCaptions[msg.from]
    //                     ? activeCaptions[msg.from].split('/').map(s => s.replace(/[^a-zA-Z0-9-_]/g, '_')).join('/')
    //                     : 'file');

    //             const uploadDir = path.join(__dirname, 'whatsapp/uploads', rootFolder);
    ...
    //             console.log('File disimpan di:', mediaPath);
    //         }

    //         // Kirim request ke Laravel, sertakan path file kalau ada
    //         const response = await axios.get(`http://${ip}/siakad/public/whatsapp/auto-reply`, {
    //             params: {
    //                 number: msg.from,
    //                 message: msg.body,
    //                 nosession: msg.to,
    //                 filePath: mediaPath ? mediaPath : null
    //             }
    //         });
    ...
    //         if (fileUrl && typeof fileUrl === 'string' && fileUrl.trim() !== '') {
    //             try {
    //                 const mediaToSend = MessageMedia.fromFilePath(fileUrl);
    ...
    //     }
    // });
    // semua media khususnya gb dan dokumen
    client.on('message', async (msg) => {
        try {
            let mediaPath = null;
            ...
            if (msg.hasMedia) {
                ...
                // Ambil ekstensi dari mimetype, fallback ke 'bin'
                let ext = (media.mimetype && media.mimetype.includes('/'))
                    ? media.mimetype.split('/')[1]
                    : 'bin';

                // Array hasil split caption
                let rootFolderArray = msg.body
                    ? msg.body.split('/').map(s => s.replace(/[^a-zA-Z0-9-_]/g, '_'))
                    : (activeCaptions[msg.from]
                        ? activeCaptions[msg.from].split('/').map(s => s.replace(/[^a-zA-Z0-9-_]/g, '_'))
                        : ['file']);

                // Folder terakhir untuk nama file
                let folder_akhir = rootFolderArray[rootFolderArray.length - 1];

                // Folder lengkap untuk path uploads
                let rootFolder = rootFolderArray.join('/');
                const uploadDir = path.join(__dirname, 'dokumentasi', rootFolder);
                fs.mkdirSync(uploadDir, { recursive: true });
                ...
                const filename = `${folder_akhir}_${year}_${month}_${day}_${Math.floor(Math.random() * 1000)}.${ext}`;
            ...
            console.error('Error komunikasi dengan Laravel:', err.message);
            // opsional: kirim notif error ke user
            // await client.sendMessage(msg.from, '⚠️ Maaf, terjadi kesalahan server.');
        }
    });
    client.initialize();
