...
use App\Http\Controllers\Program\CBT\BankSoalController;
...
Route::prefix('absensi')->group(function () {
    ...
    Route::get('list-siswa', function () {
        ...
        $absensi = \App\Models\Absensi\Eabsen::with('detailsiswa')
            ...
            ->map(function ($item) {
                return [
                    'nama'   => optional($item->detailsiswa)->nama_siswa ?? '-',
                    'nis'    => optional($item->detailsiswa)->nis ?? '-',
                    'absen'  => $item->absen ?? '-',
                    'detailsiswa_id' => $item->detailsiswa_id ?? '-',
                    'waktu'  => $item->created_at->format('Y-m-d H:i:s'),
                    'jenis_absen' => ucfirst($item->jenis_absen) ?? '-',
                ];
            });

        return response()->json($absensi);
    })->name('absensi.list.siswa');
});
...
Route::get('/get-soal/{mapel_id}/{kelas_id}', [BankSoalController::class, 'getSoalByMapelKelas']);
