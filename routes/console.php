...
Artisan::command('make:add-migration {name} {table} {array_field}', function ($name, $table, $array_field) {
    $timestamp = date('Y_m_d_His');
    $classBaseName = Str::studly($name);
    $fileName = $timestamp . '_' . Str::snake($name) . '.php';
    $dirPath = database_path('migrations');
    $filePath = $dirPath . '/' . $fileName;

    if (File::exists($filePath)) {
        $this->error("❌ File migration '{$fileName}' sudah ada.");
        return;
    }

    // --- ubah array_field string "nama_ibu:penghasilan_ibu:alamat_ibu" jadi array ---
    $columns = explode(':', $array_field);

    // Buat string PHP untuk up()
    $columnsPHP = [];
    foreach ($columns as $col) {
        $columnsPHP[] = "['name' => '$col', 'type' => 'string', 'nullable' => true]";
    }
    $columnsPHPString = implode(",\n                ", $columnsPHP);

    // Buat string PHP untuk down() → array nama kolom
    $columnsDownString = implode("','", $columns); // "nama_ibu','penghasilan_ibu','alamat_ibu"
    $columnsDownString = "'" . $columnsDownString . "'"; // tambah quotes di awal & akhir
        ...
        Schema::table('{$table}', function (Blueprint \$table) {
            ...
            foreach (\$columns as \$col) {
                if (!Schema::hasColumn('{$table}', \$col['name'])) {
                    switch (\$col['type']) {
                        case 'string': \$c = \$table->string(\$col['name']); break;
                        case 'integer': \$c = \$table->integer(\$col['name']); break;
                        case 'date': \$c = \$table->date(\$col['name']); break;
                        default: throw new Exception("Tipe kolom {\$col['type']} belum didukung.");
                    }
                    if (!empty(\$col['nullable'])) \$c->nullable();
                    echo "✔ Kolom {\$col['name']} berhasil ditambahkan.\\n";
                } else {
                    echo "ℹ Kolom {\$col['name']} sudah ada, dilewati.\\n";
                }
            }
        });
    }
...
PHP;
    ...
    File::ensureDirectoryExists($dirPath);
    File::put($filePath, $template);
    $this->info("✅ Migration '{$classBaseName}' berhasil dibuat di {$filePath}");
})->describe('Buat migration custom dengan array_field & pengecekan kolom');
...
/*
    ...
    |
    | Tujuan :
    | - Dugunakan untuk menambah kolom / field
    |
    |
    | Penggunaan :
    | - xxxxxxxxxxx
    ...
    */
// Proses Coding
// php artisan make:add-migration siswa/tambah_data_ortu detailsiswas nama_ibu:penghasilan_ibu:alamat_ibu
...
Artisan::command('make:add-migration {name} {table} {array_field}', function ($name, $table, $array_field) {
    ...
    $name = str_replace('\\', '/', $name);
    $parts = explode('/', $name);
    $classBaseName = Str::studly(array_pop($parts)); // nama kelas migration
    $subPath = implode('/', $parts); // subfolder, bisa kosong

    $fileName = $timestamp . '_' . Str::snake($classBaseName) . '.php';
    $dirPath = database_path('migrations' . ($subPath ? "/$subPath" : '')); // gabung subfolder
    $filePath = $dirPath . '/' . $fileName;
    ...
    // --- ubah array_field string jadi array ---
    $columns = explode(':', $array_field);
    ...
    // buat string untuk down()
    $columnsDownString = "'" . implode("','", $columns) . "'";
...
PHP;
    ...
    // --- pastikan folder ada sebelum write ---
    File::ensureDirectoryExists($dirPath);
    File::put($filePath, $template);

    $this->info("✅ Migration '{$classBaseName}' berhasil dibuat di {$filePath}");
})->describe('Buat migration custom dengan array_field & pengecekan kolom dan subfolder');
