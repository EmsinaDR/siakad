                                    ...
                                    <tr>
                                        ...
                                        <td class='text-left'>
                                            @php
                                                ...
                                                $data->daftar_guru = \App\Models\User\Guru\Detailguru::whereIn(
                                                    'id',
                                                    $decoded_ids,
                                                )->get();
                                            @endphp
                                            <ul>
                                                @forelse ($data->daftar_guru as $guru)
                                                    <li>{{ $guru->nama_guru }}{{ $guru->gelar ? ', ' . $guru->gelar : '' }}
                                                    </li>
                                                @empty
                                            ...
                                            @if ($Identitas->paket !== 'Gratis')
                                                <div class='gap-1 d-flex justify-content-center'>
                                                    ...
                                                    <form id='delete-form-{{ $data->id }}'
                                                        action='{{ route('seting-pengguna-program.destroy', $data->id) }}'
                                                ...
                                                <x-edit-modal>
                                                    ...
                                                    <section>
                                                        <form id='updateurl'
                                                            ...
                                                            <x-inputallin>readonly:Nama
                                                                Program:Label:Placeholder:Name:Program
                                                                {{ $data->nama_program }}:readonly</x-inputallin>
                                                            @php
                                                            ...
                                                            <div class='form-group'>
                                                                ...
                                                                <select id='id_detailguru_{{ $data->id }}'
                                                                    name='detailguru_id[]' class='form-control select2'
                                                                    multiple="multiple" data-placeholder="Pilih Guru"
                                                                    data-preselect='@json($datachoce)'
                                                                    style="width: 100%;">
                                                                    @foreach ($dataGuru->whereNotIn('id', [1, 2, 3]) as $newnama)
                                                                        <option value='{{ $newnama->id }}'>
                                                                            {{ $newnama->nama_guru }}{{ $newnama->gelar ? ', ' . $newnama->gelar : '' }}
                                                                        </option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                            ...
                                                            <script>
                                                                $(document).on('shown.bs.modal', '.modal', function() {
                                                                    ...
                                                                    $modal.find('.select2').each(function() {
                                                                        const $select = $(this);
                                                                        const preselectData = $select.data('preselect');

                                                                        // Hancurkan dulu kalau sudah pernah diinit
                                                                        ...
                                                                        $select.select2({
                                                                            ...
                                                                            width: '100%',
                                                                            placeholder: $select.data('placeholder') || 'Pilih Guru'
                                                                        });

                                                                        // Jika ada preselect, set val dulu tapi TUNGGU sedikit biar modal settle
                                                                        if (Array.isArray(preselectData) && preselectData.length > 0) {
                                                                            setTimeout(() => {
                                                                                $select.val(preselectData).trigger('change');
                                                                                // Paksa render ulang posisi dropdown setelah terpilih
                                                                                $select.select2('close'); // pastikan tertutup dulu
                                                                                $select.select2('open'); // buka sebentar biar posisi hitung ulang
                                                                                $select.select2('close');
                                                                            }, 250); // delay 250ms, aman di semua browser
                                                                        }
                                                                    });
                                                                });
                                                            </script>
                                                            ...
                                                            <style>
                                                                /* Tambahan jaga-jaga kalau masih nembus */
                                                                .select2-container {
