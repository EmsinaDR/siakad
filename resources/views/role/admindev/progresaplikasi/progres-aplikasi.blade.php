                            ...
                            <tbody>
                                @foreach ($datas as $data)
                                    <tr>
                                        ...
                                        <!-- Menampilkan judul -->
                                        <td class="text-center">{{ $data->role ?? '-' }}</td> <!-- Menampilkan role -->
                                        <td class="text-center">{{ $data->fitur ?? '-' }}</td>
                                        <!-- Menampilkan fitur -->
                                        <td class="text-center">{{ $data->model_class ?? '-' }}</td>
                                        <!-- Menampilkan model_class, jika ada -->
                                        <td class="text-center">{{Carbon::create($data->tanggal_mulai ?? now())->translatedformat('l, d F Y')}}</td>
                                        <!-- Menampilkan tanggal mulai, jika ada, jika tidak, tampilkan waktu sekarang -->
                                        <td class="text-center">{{Carbon::create($data->tanggal_akhir ?? now())->translatedformat('l, d F Y')}}</td>
                                        <!-- Menampilkan tanggal akhir, jika ada, jika tidak, tampilkan waktu besok -->
                                        <td class="text-center">{{ $data->persentase ?? 0 }}</td>
                                        <!-- Menampilkan persentase, jika ada, jika tidak, tampilkan 0 -->
                                        <td class="text-left">{!! $data->keterangan ?? '-' !!}</td>
                                        <td class="text-left">{{ $data->keterangan ?? '-' }}</td>
                                        <!-- Menampilkan keterangan, jika ada, jika tidak, tampilkan '-' -->
                                        ...
                                        <td width='10%'>
                                            {{-- blade-formatter-disable --}}
                                                   <div class='gap-1 d-flex justify-content-center'>
                                                       ...
                                                       <a href="{{route('copy-fitur', $data->id)}}"><button type='button' class='btn btn-warning btn-sm btn-equal-width' data-toggle='modal' data-target='#editModal{{ $data->id }}'><i class='fa fa-copy'></i> </button></a>
                                                       <!-- Form untuk menghapus -->
                                                       <form id='delete-form-{{ $data->id }}'  action='{{ route('progres-aplikasi.destroy', ['progres_aplikasi' => $data->id]) }}' method='POST' style='display: inline-block;'>
                                                           @csrf
                                                           @method('DELETE')
                                                       </form>
                                            ...
                                            <div class='modal fade' id='editModal{{ $data->id }}' tabindex='-1'
                                                ...
                                                <x-edit-modal>
                                                    ...
                                                    <section>
                                                        <form id='updateurl'
                                                            action='{{ route('progres-aplikasi.update', $data->id) }}'
                                                            method='POST'>
                                                            @csrf
                                                            @method('PATCH')
...
<script>
    $(document).ready(function() {
        initDataTable('#example1', 'Data Progres', '#example1_wrapper .col-md-6:eq(0)');
    ...
    function initDataTable(tableId, exportTitle, buttonContainer) {
        ...
        var table = $(tableId).DataTable({
            ...
            columnDefs: [{
                targets: [9], // Kolom terakhir (Action) bisgunkan array contoh [-1, -2, -4]
