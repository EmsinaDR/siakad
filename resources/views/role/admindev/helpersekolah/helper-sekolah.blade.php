...
<x-layout>
    ...
    <section class='content mx-2 my-4'>
        ...
        <div class='card'>
            ...
            <tbody>
                   @foreach ($HelperSekolah as $data)
                   <tr>
                       <td class='text-center'>{{ $loop->iteration }}</td>
                       <td class='text-center'> {{ $data->judul}}</td><td class='text-center'> {{ $data->slug}}</td><td class='text-center'> {{ $data->content}}</td><td class='text-center'> {{ $data->keterangan}}</td>
                       ...
                       <td width='10%'>
                                            ...
                                            <div class='d-flex justify-content-center'>
                                                ...
                                                <form id='delete-form-{{ $data->id }}' action='{{ route('helper-auto-reply-whatsapp.destroy', $data->id) }}' method='POST' style='display: inline-block;'>
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                              ...
                              <div class='modal fade' id='editModal{{ $data->id }}' tabindex='-1' aria-labelledby='EditModalLabel' aria-hidden='true'>
                                  <x-edit-modal>
                                      ...
                                      <section>
                                             <form id='update-helper-auto-reply-whatsapp' action='{{ route('helper-auto-reply-whatsapp.update', $data->id) }}' method='POST'>
                                                 @csrf
                                                 @method('PATCH')

                                                    <div class='form-group'>    <label for='{{datafill}}'></label>    <input type='text' class='form-control' id='{{datafill}}' name='{{datafill}}' placeholder='placeholder' value='{{ $data->judul}}' required></div><div class='form-group'>    <label for='{{datafill}}'></label>    <input type='text' class='form-control' id='{{datafill}}' name='{{datafill}}' placeholder='placeholder' value='{{ $data->slug}}' required></div><div class='form-group'>    <label for='{{datafill}}'></label>    <input type='text' class='form-control' id='{{datafill}}' name='{{datafill}}' placeholder='placeholder' value='{{ $data->content}}' required></div><div class='form-group'>    <label for='{{datafill}}'></label>    <input type='text' class='form-control' id='{{datafill}}' name='{{datafill}}' placeholder='placeholder' value='{{ $data->keterangan}}' required></div>
    ...
    <div class='modal-dialog modal-lg'>
           <div class='modal-content'>
               ...
               <div class='modal-body'>
                   ...
                   <form id='TambahData-form' action='{{route('helper-auto-reply-whatsapp.store')}}' method='POST'>
                          @csrf
                          @method('POST')
