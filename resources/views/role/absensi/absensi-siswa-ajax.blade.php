...
function loadAbsenList() {
        ...
        axios.get(absenListUrl)
            .then(response => {
                ...
                response.data.forEach(d => {
                    ...
                    rows += `
                        <tr>
                            <td>( <span class="badge bg-primary">${d.detailsiswa_id ?? '-'}</span> ) ${d.nama ?? '-'}</td>
                            <td>${d.nis ?? '-'}</td>
                            <td class="text-center">${d.absen ? d.absen.charAt(0).toUpperCase() + d.absen.slice(1) : '-'}</td>
                            <td class="text-center">${d.jenis_absen ? d.jenis_absen.charAt(0).toUpperCase() + d.jenis_absen.slice(1) : '-'}</td>
                            <td>${d.waktu ?? '-'}</td>
                        </tr>
                    `;
                });
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function onScanSuccess(decodedText) {
                ...
                axios.post("{{ route('absensi.store.siswa.ajax') }}", {
                    ...
                    .then(response => {
                        ...
                        if (!res.success) {
                            resultDiv.textContent = res.message || "Terjadi kesalahan";
                            return;
                        }

                        resultDiv.textContent = "Absensi berhasil untuk " + res.data.nama;

                        loadAbsenList();

                    })
                    .catch(err => {
                        resultDiv.textContent = "Terjadi kesalahan server";
                    })
                    .finally(() => {
            ...
            loadAbsenList();
            setInterval(loadAbsenList, 10000);
