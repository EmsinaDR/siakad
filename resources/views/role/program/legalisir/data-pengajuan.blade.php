                            ...
                            <tbody>
                                @foreach ($datas as $data)
                                    <tr>
                                        ...
                                        <td class='text-center'> {{ $data->jumlah_lembar }}</td>
                                        <td class='text-left'> {{ $data->keperluan }}</td>
                                        <td class='text-center'>
                                            @if ($data->status === 'menunggu')
                                                <span
                                                    class="p-2 rounded-pill bg-secondary">{{ ucfirst($data->status) }}</span>
                                            @elseif($data->status === 'diproses')
                                                <span
                                                    class="p-2 rounded-pill bg-primary">{{ ucfirst($data->status) }}</span>
                                            @elseif($data->status === 'selesai')
                                                <span
                                                    class="p-2 rounded-pill bg-success">{{ ucfirst($data->status) }}</span>
                                            @else
                                                <span
                                                    class="p-2 rounded-pill bg-danger">{{ ucfirst($data->status) }}</span>
                                            @endif
                                        </td>
                                        ...
                                        <td width='10%'>
                                            <div class='gap-1 d-flex justify-content-center'>
                                                ...
                                                <form id='delete-form-{{ $data->id }}'
                                                    action='{{ route('legalisir-ijazah.destroy', $data->id) }}'
                                            ...
                                            <div class='modal fade' id='editModal{{ $data->id }}' tabindex='-1'
                                                ...
                                                <x-edit-modal>
                                                    ...
                                                    <section>
                                                        <form id='updateurl'
                                                            action='{{ route('legalisir-ijazah.update', $data->id) }}'
                                                            method='POST'>
                                                            @csrf
                                                            @method('PATCH')
                                                            @php
                                                            $Statuss = ['menunggu', 'diproses', 'selesai', 'ditolak'];
                                                            @endphp

                                                            {{-- blade-formatter-disable --}}
                                                            <div class='form-group'>
                                                                <label for='status'>Status Proses</label>
                                                                <select name='status' id='id_status-{{ $data->id }}'class='form-control' required>
                                                            ...
                                                            <script>
                                                               $(document).ready(function() {
                                                                   ...
                                                                   $('#id_status-{{ $data->id }}').val(@json($data->status)).trigger('change'); // Mutiple Select Select value in array json
                                                               });
                                                            </script>
