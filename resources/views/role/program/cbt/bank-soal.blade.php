                ...
                <div class='card'>
                    ...
                    <div class='card-body'>
                        ...
                        <table id='example1' width='100%' class='table table-responsive table-bordered table-hover'>
                            ...
                            <tbody>
                                @foreach ($DataBankSoal as $data)
                                    <tr>
                                        <td class='text-center'>{{ $loop->iteration }}</td>
                                        <td class='text-center'> {{ $data->mapel->mapel }}</td>
                                        <td class='text-center'> {{ $data->soal }}</td>
                                        <td class='text-center'> {{ $data->level }}</td>
                                        <td class='text-center'> {{ $data->jawabana }}</td>
                                        <td class='text-center'> {{ $data->jawabanb }}</td>
                                        <td class='text-center'> {{ $data->jawabanc }}</td>
                                        <td class='text-center'> {{ $data->jawaband }}</td>
                                        ...
                                        <td width='10%'>
                                            {{-- blade-formatter-disable --}}
                                                   <div class='gap-1 d-flex justify-content-center'>
                                                       ...
                                                       <form id='delete-form-{{ $data->id }}' action='{{ route('bank-soal.destroy', $data->id) }}' method='POST' style='display: inline-block;'>
                                                           @csrf
                                                           @method('DELETE')
                                                       </form>
                                            ...
                                            <div class='modal fade' id='editModal{{ $data->id }}' tabindex='-1'
                                                ...
                                                <x-edit-modal>
                                                    ...
                                                    <section>
                                                        <form id='updateurl'
                                                            action='{{ route('bank-soal.update', $data->id) }}'
                                                            method='POST'>
                                                            @csrf
                                                            @method('PATCH')
