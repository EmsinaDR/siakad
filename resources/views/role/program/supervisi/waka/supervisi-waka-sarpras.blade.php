    ...
    <section class='content mx-2 my-4'>
        ...
        <div class='card'>
            ...
            {{-- blade-formatter-disable --}}
               <div class='row m-2'>
                   <div class='col-xl-2'>
                       <form action="{{ route('supervisi-waka-sarpras.store') }}"
                            method="POST">
                            @csrf
                            @method('POST')
                            <input type="hidden" name="bidang" value="Waka Sarpras">
                            ...
                            <tbody>
                                @foreach ($datas as $data)
                                    <tr>
                                        ...
                                        <td class='text-center' width='10%'> {{ $data->Tapel->tapel }} /
                                            {{ $data->Tapel->tapel + 1 }}</td>
                                        ...
                                        <td width='10%'>
                                            <div class='gap-1 d-flex justify-content-center'>
                                               ...
                                               {{-- blade-formatter-disable --}}
                                                <form action="{{ route('supervisi-waka-sarpras.store') }}"
                                                    ...
                                                    @method('POST')
                                                    <input type="hidden" name="bidang" value="Waka Sarpras">
                                                    <button type="submit" class='btn btn-primary btn-sm btn-equal-width'><i class="fa fa-edit"></i></button>
                                                </form>
                                               {{-- blade-formatter-enable --}}
                                                <!-- Form untuk menghapus -->
                                                <form id='delete-form-{{ $data->tapel_id }}'
                                                    action='{{ route('supervisi-waka-sarpras.destroy', $data->tapel_id) }}'
                                            ...
                                            <div class='modal fade' id='editModal{{ $data->id }}' tabindex='-1'
                                                ...
                                                <x-edit-modal>
                                                    ...
                                                    <section>
                                                        <form id='updateurl'
                                                            action='{{ route('supervisi-waka-sarpras.update', $data->tapel_id) }}'
                                                            method='POST'>
                                                            @csrf
                                                            @method('PATCH')
