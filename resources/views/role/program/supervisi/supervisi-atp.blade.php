                                ...
                                <tr>
                                    ...
                                    <td width='10%'>
                                        <div class='gap-1 d-flex justify-content-center'>
                                            <form action="{{ route('supervisi-modul-ajar-guru.store') }}"
                                                method="POST">
                                                @csrf
                                                @method('POST')
                                        ...
                                        <form id='delete-form-{{ $data->id }}'
                                            action='{{ route('supervisi-modul-ajar-guru.destroy', $data->id) }}'
                                            method='POST' style='display: inline-block;'>
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                        <button type='button' class='btn btn-danger btn-sm btn-equal-width' onclick='confirmDelete({{ $data->id }})'> <i class='fa fa-trash'></i>
                                        </button>
                                        {{-- blade-formatter-enable --}}
                                        </div>
                                        ...
                                        <div class='modal fade' id='editModal{{ $data->id }}' tabindex='-1'
                                            ...
                                            <x-edit-modal>
                                                ...
                                                <section>
                                                    <form id='updateurl'
                                                        action='{{ route('supervisi-modul-ajar-guru.update', $data->id) }}'
                                                        method='POST'>
                                                        @csrf
                                                        @method('PATCH')
