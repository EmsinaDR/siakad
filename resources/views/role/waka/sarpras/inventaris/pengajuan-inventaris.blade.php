                ...
                <div class='card-body'>
                    ...
                    <div class="col-xl-10">
                        <table id='example1x' width='100%' class='table table-bordered table-hover'>
                            ...
                            <tbody>
                                {{-- @foreach ($datas as $kode_ruangan => $items)
                                    <tr class="bg-gray-200">
                                        <td colspan="6" class="text-left font-bold">Ruangan: {{ $kode_ruangan }}</td>
                                    </tr>
                                    @foreach ($items as $data)
                                        <tr class='text-center'>
                                            <td>{{ $loop->iteration }}</td>
                                            <td>{{ $data->nama_barang }} / {{ $data->kode_ruang }} /
                                                {{ $data->kode_ruangan }}</td>
                                            <td>{{ $data->jumlah }}</td>
                                            <td>{{ $data->harga }}</td>
                                ...
                                @foreach ($datas as $kode_ruangan => $items)
                                    @php
                                        $totalHarga = $items->sum(fn($item) => $item->jumlah * $item->harga);
                                    @endphp
                                    <tr class="bg-gray-200">
                                        <td colspan="6" class="text-left font-bold">Ruangan: {{ $kode_ruangan }}</td>
                                    </tr>
                                    ...
                                    <tr class="bg-gray-300 font-bold">
                                        <td colspan="4" class="text-right">Total Ruangan {{ $kode_ruangan }}</td>
