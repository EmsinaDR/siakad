...
<x-layout>
    ...
    <section class='content mx-2 my-4'>
        ...
        <div class='card'>
            ...
            <tbody>
                   @foreach ($DokumenSiswa as $data)
                   <tr>
                       <td class='text-center'>{{ $loop->iteration }}</td>
                       <td class='text-center'> {{ $data->kk:ayah_ktp:ibu_ktp:ijazah:ijazah_sd:raport}}</td>
                       ...
                       <td width='10%'>
                              ...
                              <div class='modal fade' id='editModal{{ $data->id }}' tabindex='-1' aria-labelledby='EditModalLabel' aria-hidden='true'>
                                  <x-edit-modal>
                                      ...
                                      <section>
                                             <form id='update-dokumen-siswa' action='{{ route('dokumen-siswa.update', $data->id) }}' method='POST'>
                                                 @csrf
                                                 @method('PATCH')

                                                    <div class='form-group'>    <label for='{{datafill}}'></label>    <input type='text' class='form-control' id='{{datafill}}' name='{{datafill}}' placeholder='placeholder' value='{{ $data->kk:ayah_ktp:ibu_ktp:ijazah:ijazah_sd:raport}}' required></div>
    ...
    <div class='modal-dialog modal-lg'>
           <div class='modal-content'>
               ...
               <div class='modal-body'>
                   ...
                   <form id='TambahData-form' action='{{route('dokumen-siswa.store')}}' method='POST'>
                          @csrf
                          @method('POST')
                         <div class='form-group'>    <label for='{{datafill}}'></label>    <input type='text' class='form-control' id='{{datafill}}' name='{{datafill}}' placeholder='placeholder' required></div>
