...
use App\Models\Bendahara\BendaharaDaftarUlang;

class BendaharaDaftarUlangController extends Controller
{
    //
    public function index(BendaharaDaftarUlang $BendaharaDaftarUlang)
    {
        $title = 'Daftar Ulang';
        $arr_ths = [
            'Nama Siswa',
        ...
        $datas = BendaharaDaftarUlang::with('BendaharaDaftarUlangtoSiswa')->where('id', 'id')->get(); //Relasi di sisipi dengan where
        ...
        return view('role.bendahara.daftarulang', compact(
            'title',
            'arr_ths',
            'datas',
            'breadcrumb',
    ...
    public function update($id, Request $request)
    {
        ...
        //dd($request->all());
        $data = $request->except(['_token', '_method']);
        $data = BendaharaDaftarUlang::findOrFail($id);
        $data->kelas_id = $request->kelas_id;
        $data->updated_at =  now();
        $data->update();
