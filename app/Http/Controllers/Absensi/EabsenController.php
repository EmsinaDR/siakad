...
class EabsenController extends Controller
{
    ...
    public function IndexAjax()
    {
        ...
        // });
        $absensi = Eabsen::with('detailsiswa')->orderByDesc('created_at')->get();
        // cek rekap
        $absensiHariIni = Eabsen::with('detailsiswa')
        ...
        $siswa = Detailsiswa::SiswaAktif()->with('kelasOne:id,kelas')
            ->select('id', 'kelas_id', 'nis', 'nama_siswa')
            ->get();
        $sudahAbsen = $absensiHariIni->pluck('detailsiswa_id')->unique()->toArray();
        $rekapPerKelas = $siswa->groupBy('kelas_id')->map(function ($kelasSiswa, $kelas_id) use ($sudahAbsen) {

            $total = $kelasSiswa->count();
            $sudah = $kelasSiswa->whereIn('id', $sudahAbsen)->count();
            $belum = $total - $sudah;

            // ambil nama kelas dari relasi (safe)
            $kelasNama = $kelasSiswa->first()->kelasOne->kelas ?? 'Tidak Ada Nama Kelas';
        ...
        return view('role.absensi.absensi-siswa-ajax', compact('absensi', 'rekapPerKelas'));
    }
    public function rekapPerKelasAjax()
    {
        $absensiHariIni = Eabsen::whereDate('created_at', today())->get();
        $siswa = Detailsiswa::with('kelasOne')->get();

        $sudahAbsen = $absensiHariIni->pluck('detailsiswa_id')->toArray();

        $rekap = $siswa->groupBy('kelas_id')->map(function ($kelasSiswa, $kelas_id) use ($sudahAbsen) {
            $total = $kelasSiswa->count();
            $sudah = $kelasSiswa->whereIn('id', $sudahAbsen)->count();
            $belum = $total - $sudah;

            return [
                'kelas_nama' => $kelasSiswa->first()->kelasOne->kelas ?? '-',
                'total_siswa' => $total,
                'sudah_absen' => $sudah,
                'belum_absen' => $belum,
            ];
    ...
    public function storeSiswaAjax(Request $request)
    {
        ...
        // ==========================
        $sudahAbsenMasuk = Eabsen::whereDate('created_at', today())
            ->where('detailsiswa_id', $siswa->id)
            ->where('jenis_absen', 'masuk')
            ->exists();

        $sudahAbsenPulang = Eabsen::whereDate('created_at', today())
            ->where('detailsiswa_id', $siswa->id)
            ->where('jenis_absen', 'pulang')
            ->exists();

        // ==========================
        // 2. Tentukan jenis absen
        // ==========================
        if (!$sudahAbsenMasuk) {
            $jenisAbsen = 'masuk';
        ...
        if (!config('whatsappSession.lockAbsensi')) {
    ...
    public function storeGuruAjaxPindah(Request $request)
    {
        $request->validate(['nis' => 'required']);
        // $datasiswa = Detailsiswa::with('Detailsiswatokelas')->where('nis', $request->datanis)->first();
        $siswa = Detailsiswa::with('Detailsiswatokelas')->where('nis', $request->nis)->first();
        ...
        $jamSekarang    = now();
        $jamBatasMasuk  = Carbon::createFromTimeString('07:05');
        $jamBatasPulang = Carbon::createFromTimeString('13:00');

        // ==========================
        // 1. Tentukan jenis absen
        // ==========================
        if ($jamSekarang->lessThanOrEqualTo($jamBatasMasuk)) {
            // Masih pagi, belum lewat jam masuk → absen masuk
            $jenisAbsen = 'masuk';
        } elseif ($jamSekarang->lessThan($jamBatasPulang)) {
            // Sudah lewat jam masuk tapi belum jam pulang → masih masuk (telat)
            $jenisAbsen = 'masuk';
        } else {
            // Sudah lewat jam pulang → absen pulang
            $jenisAbsen = 'pulang';
        }

        // ==========================
        // 2. Cek sudah absen belum
        // ==========================
        $sudahAbsen = Eabsen::whereDate('created_at', today())
            ->where('detailsiswa_id', $siswa->id)
            ->where('jenis_absen', $jenisAbsen)
        ...
        if ($jenisAbsen === 'masuk') {
            ...
            if ($jamSekarang->lessThan($jamBatasPulang)) {
                $pulangCepat = $jamSekarang->diffInMinutes($jamBatasPulang);
            } elseif ($jamSekarang->greaterThan($jamBatasPulang)) {
                $pulangTelat = $jamBatasPulang->diffInMinutes($jamSekarang);
            }
        }
