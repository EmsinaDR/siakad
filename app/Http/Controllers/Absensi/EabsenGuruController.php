...
class EabsenGuruController extends Controller
{
    ...
    public function storeGuruAjax(Request $request)
    {
        ...
        // ==========================
        $absenMasuk = EabsenGuru::whereDate('created_at', today())
            ->where('detailguru_id', $guru->id)
            ->where('jenis_absen', 'masuk')
            ->first();

        $absenPulang = EabsenGuru::whereDate('created_at', today())
            ->where('detailguru_id', $guru->id)
        ...
        $PesanKirim =
            "{$pulangTelat} \n" .
            "\n\n";
        $result = \App\Models\Whatsapp\WhatsApp::sendMessage('GuruId', '6285329860005', format_pesan('judul', $PesanKirim));
        return false;
