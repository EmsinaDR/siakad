...
class WaliKelasController extends Controller
{
    ...
    public function Walkes_DataAbsensi(Request $request)
    {
        ...
        $datas = DB::table('detailsiswas')
            ->leftJoin('eabsens', function ($join) {
                $join->on('detailsiswas.id', '=', 'eabsens.user_id')
                    // ->where('eabsens.absen', '=', 'hadir'); // Filter hanya yang absen
                    // ->whereDate('eabsens.waktu_absen', now()->toDateString());
                    ->whereDate('eabsens.waktu_absen', '=', date('Y-m-d')); // Hanya absen hari ini
            })
            ->where('detailsiswas.kelas_id', $request->kelas_id) // Ambil hanya kelas A
            // ->leftJoin('Ekelas', 'Ekelas.id', '=', 'detailsiswas.kelas_id')
            ->select('detailsiswas.id', 'detailsiswas.nis', 'detailsiswas.nama_siswa', 'detailsiswas.kelas_id', 'eabsens.absen', 'eabsens.waktu_absen', 'eabsens.waktu_absen')
    ...
    public function Walkes_DataRekapAbsensi(Request $request)
    {
        ...
        $datas = Eabsen::get();
        $tahun = 2024; // Variabel untuk tahun yang dinamis
        $bulanArray = [1, 2, 3, 4, 5, 6]; // Daftar bulan yang diinginkan

        $datas = Eabsen::whereYear('waktu_absen', $tahun)
            ->whereMonth('waktu_absen', '<=', 6)
            ->where('kelas_id', 1) // Hanya bulan 1-6
            ->get()
            ->groupBy('user_id')
            ->map(function ($absen) use ($tahun, $bulanArray) {
                $data = ['user_id' => $absen->first()->user_id];

                foreach ($bulanArray as $bulan) {
                    $firstDayOfMonth = Carbon::create($tahun, $bulan, 1)->startOfMonth()->toDateString();
                    $lastDayOfMonth = Carbon::create($tahun, $bulan, 1)->endOfMonth()->toDateString();
                    $bulanName = Carbon::parse($firstDayOfMonth)->format('M'); // Mengambil nama bulan

                    // Menambahkan data absen per bulan
                    $data['H_' . $bulanName] = $absen->whereBetween('waktu_absen', [$firstDayOfMonth, $lastDayOfMonth])->where('absen', 'hadir')->count();
                    $data['S_' . $bulanName] = $absen->whereBetween('waktu_absen', [$firstDayOfMonth, $lastDayOfMonth])->where('absen', 'sakit')->count();
                    $data['I_' . $bulanName] = $absen->whereBetween('waktu_absen', [$firstDayOfMonth, $lastDayOfMonth])->where('absen', 'ijin')->count();
                    $data['A_' . $bulanName] = $absen->whereBetween('waktu_absen', [$firstDayOfMonth, $lastDayOfMonth])->where('absen', 'alfa')->count();
                }
            ...
            ->values();
        //compact => 'title', 'arr_ths', 'breadcrumb', 'titleviewModal', 'titleeditModal', 'titlecreateModal'
        return view('role.walkes.data-rekap-absensi', compact('datas', 'title', 'arr_ths', 'breadcrumb', 'titleviewModal', 'titleeditModal', 'titlecreateModal'));
    }
