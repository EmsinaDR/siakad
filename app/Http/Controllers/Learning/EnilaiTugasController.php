...
// use App\Http\Controllers\Learning\Etugas;

class EnilaiTugasController extends Controller
{
    //
    public function index(Request $request)
    {
        ...
        return view('role.guru.e_nilai_tugas', compact('title', 'arr_ths', 'datas', 'breadcrumb', 'titleviewModal', 'titleeditModal', 'titlecreateModal'));
        // return 'oke';
    }
    ...
    public function store(Request $request)
    {
        ...
        $etapels = Etapel::where('aktiv', 'Y')->first();
        $data = Etugas::where('kelas_id', $request->kelas_id)->where('tapel_id', $etapels->id)->where('semester', $etapels->semester)->where('mapel_id', $request->mapel_id)->first();
        dd($request->all());
        $data = $request->except(['_token', '_method']);
        $data = new EnilaiTugas();
    ...
    public function update($id, Request $request)
    {
        ...
        $data = EnilaiTugas::findOrFail($request->nilai_tuga);
        $data->tugasa = $request->tugasa;
        $data->tugasb = $request->tugasb;
        $data->tugasc = $request->tugasc;
        $data->tugasd = $request->tugasd;
        $data->tugase = $request->tugase;
        $data->updated_at =  now();
        $data->update();
        // dd($request->all());
    ...
    public function upadaAllIn_tugas($id, Request $request)
    {
        // dd($request->all());
        ...
        for ($i = 0; $i < count($request->id_data); $i++):
            $data = EnilaiTugas::findOrFail($request->id_data[$i]);
            $data->tugasa = $request->tugasa[$i];
            $data->tugasb = $request->tugasb[$i];
            $data->tugasc = $request->tugasc[$i];
            $data->tugasd = $request->tugasd[$i];
            $data->tugase = $request->tugase[$i];
    ...
    public function DS_generatsiswa($id, Request $request)
    {
        ...
        try {
            ...
            foreach ($siswas as $siswa):
                ...
                $data->created_at = now();
                $data->updated_at =  now();
                $ceksiswa = EnilaiTugas::where('kelas_id', $request->kelas_id)->where('tapel_id', $etapels->id)->where('semester', $etapels->semester)->where('mapel_id', $request->mapel_id)->where('detailsiswa_id', $siswa->id)->count();
                // dd($ceksiswa);
                if ($ceksiswa === 0):
                    $data->save();
                endif;
            ...
            return Redirect::back()->with('Title', 'Gagal')->with('gagal', 'Gagal <li>Kelas tidak ada</li><li>tidak dikenali</li>');
        }
    }
