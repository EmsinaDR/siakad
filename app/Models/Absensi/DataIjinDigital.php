...
class DataIjinDigital extends Model
{
    ...
    public static function createByWa($nohp, $pesan) // Untuk create di Whatsapp GateWay
    {
        ...
        $siswa = Detailsiswa::with('kelasOne')->where('nis', $part2)->first();
        if (!$siswa) {
        ...
        // Kirim Kew Wali Kelas
        $kelas = $siswa->kelasOne->kelas;
        $PesanWalkes =
            "Di Informasikan kepada Bapak / Ibu Wali Kelas {$kelas} terkait dengan ijin digital :\n" .
            "Nama    : {$siswa->nama_siswa}\n" .
            "Kelas    : {$kelas}\n" .
            "Melalui aplikasi ijin digital meminta ijin tidak hadir dikarenakan *{$part4}*\n" .
            "Demikian informasi ini disampaikan.\n" .
            "Atas perhatian Bapak / Ibu Wali Kelas, kami sampaikan terima kasih.\n" .
            "\n";
        $walikelas = $siswa->kelasOne->Guru->nama_guru;
        $NoHPWalkes = $siswa->kelasOne->Guru->no_hp;
        $result = \App\Models\Whatsapp\WhatsApp::sendMessage($sessions, $NoHPWalkes,  format_pesan('Informasi Ijin Digital', $PesanWalkes));
        return $data;
    }
}
