...
class GuruPiketCommand extends Command
{
    ...
    public function handle()
    {
        ...
        if ($Identitas->paket === 'Premium') {
            ...
            if (is_day('senin')) {
                $kodeGuruPiket = ['JM', 'SA', 'AS'];
            } elseif (is_day('selasa')) {
                $kodeGuruPiket = ['DT', 'NQ', 'AS'];
            } elseif (is_day('rabu')) {
                $kodeGuruPiket = ['ER', 'UN', 'AS'];
            } elseif (is_day('kamis')) {
                $kodeGuruPiket = ['MA', 'CY', 'AS'];
            } elseif (is_day('jumat')) {
                $kodeGuruPiket = ['IM', 'SZ', 'AS'];
            } elseif (is_day('sabtu')) {
                $kodeGuruPiket = ['IZ', 'AH', 'AS'];
            } else {
                $kodeGuruPiket = [];
            }
        ...
        foreach ($siswaTidakAbsen as $kelas => $daftarSiswa) {
            ...
            foreach ($daftarSiswa as $siswa) {
                $namaSiswa .= "- {$siswa->nama_siswa} -> (id : {$siswa->id})\n";
            }
        ...
        */
        ...
        foreach ($jadwalHariIni as $jadwal) {
            ...
            foreach ($GuruPertama as $guru) {
                ...
                \App\Models\Whatsapp\WhatsApp::sendMessage(
                    $sessions,
                    $NoTujuan,
                    format_pesan('Informasi Guru Jam Pertama', $PesanIkiramn)
                );
        ...
        $this->info("Command nformasi Guru Jam Pertama selesai dijalankan untuk hari " . strtoupper($hariIni ?? '???'));
    }
}
