...
// dokumen gmabar to pdf

if (!function_exists('generate_pdf_from_images')) {
    /**
     * Buat PDF dari array gambar dan simpan ke folder
     *
     * @param array $images Array path gambar
     * @param string|null $savePath Path lengkap tempat simpan PDF. Kalau null, bikin otomatis
     * @param string $paper Ukuran kertas (default A4)
     * @param string $orientation portrait/landscape
     * @return string|null Path PDF yang berhasil dibuat, null kalau gagal
     */
    /*
        penggunaan :
            $images = [
            public_path('images/img1.png'),
            public_path('images/img2.png'),
            public_path('images/img3.png'),
        ];

        // Misal nis siswa: 12345
        $nis = '12345';
        $folder = public_path("dokumen/siswa/$nis");
        ...
        // Panggil helper
        $pathPdf = generate_pdf_from_images($images, $savePath);

        if ($pathPdf) {
            // Tampilkan URL supaya bisa diakses browser
            $url = url(str_replace(public_path(), '', $pathPdf));
            echo "PDF berhasil dibuat: <a href='$url' target='_blank'>$url</a>";
        } else {
            echo "Gagal membuat PDF!";
        }

    */
    function generate_pdf_from_images(array $images, string $savePath = null, string $paper = 'A4', string $orientation = 'portrait'): ?string
    {
        $options = new Options();
        ...
        foreach ($images as $index => $img) {
            ...
            $html .= '<img src="' . $img . '" style="width:500px; height:auto;">';
            $html .= '</div>';
        ...
        if (!$savePath) {
            $folder = public_path('temp/dokumen');
            if (!file_exists($folder)) {
                mkdir($folder, 0777, true);
            }
            $savePath = $folder . '/pdf_' . time() . '.pdf';
        } else {
        ...
        $result = file_put_contents($savePath, $dompdf->output());
