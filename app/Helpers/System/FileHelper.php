...
if (!function_exists('copyDb')) {
    /**
     * Menyalin file backup database dari storage/backups/sql ke storage/temp,
     * lalu ke public/temp.
     *
     * @param string $namaFile Nama file (contoh: backup_20251013.sql atau .zip)
     * @return array ['status' => 'success|error', 'message' => string, 'public_path' => string|null]
     */
    function copyDb(string $namaFile): array
    {
        // Path dasar
        $source = storage_path('backups/sql/' . $namaFile);
        $tempStorage = storage_path('temp/' . $namaFile);
        $publicTemp = public_path('temp/' . $namaFile);

        // Cek file sumber
        if (!file_exists($source)) {
            return ['status' => 'error', 'message' => "❌ File tidak ditemukan di: $source"];
        }

        // Pastikan folder tujuan ada
        if (!is_dir(dirname($tempStorage))) mkdir(dirname($tempStorage), 0775, true);
        if (!is_dir(dirname($publicTemp))) mkdir(dirname($publicTemp), 0775, true);

        // 1️⃣ Copy ke storage/temp
        if (!@copy($source, $tempStorage)) {
            $err = error_get_last();
            return ['status' => 'error', 'message' => "Gagal salin ke storage/temp: " . ($err['message'] ?? '-')];
        }

        // 2️⃣ Copy ke public/temp
        if (!@copy($tempStorage, $publicTemp)) {
            $err = error_get_last();
            return ['status' => 'error', 'message' => "Gagal salin ke public/temp: " . ($err['message'] ?? '-')];
        }

        return [
            'status' => 'success',
            'message' => "✅ File berhasil disalin ke public/temp/",
            'public_path' => $publicTemp
        ];
    }
}
