...
if (!function_exists('render_svg_base64')) {
    /**
     * Render file SVG jadi base64 inline image dengan penggantian variabel dinamis.
     *
     * @param string $kode        Nama file SVG tanpa ekstensi (.svg)
     * @param array  $variables   Data yang akan menggantikan placeholder {key}
     * @param string|null $pathMaster  Path dasar tempat SVG berada (relative ke /public)
     * @return string             Data URI image/svg+xml dalam bentuk base64
     */
    function render_svg_base64($kode, $variables = [], $pathMaster = null)
    {
        // 🗂️ Default path kalau tidak diisi
        $pathMaster = $pathMaster ?: 'img/template/cocard';

        // 🔍 Bangun path lengkap ke file SVG
        $templatePath = public_path(trim($pathMaster, '/\\') . '/' . $kode . '.svg');

        if (!file_exists($templatePath)) {
            return '<!-- Template SVG ' . e($kode) . ' tidak ditemukan di ' . e($pathMaster) . ' -->';
        }

        // 📄 Baca isi SVG
        $svg = file_get_contents($templatePath);

        // 🔁 Ganti setiap placeholder {key} dengan data yang dikirim
        foreach ($variables as $key => $value) {
            // Jika value adalah path gambar (png, jpg, jpeg, svg)
            if (is_string($value) && preg_match('/\.(png|jpe?g|svg)$/i', $value)) {
                ...
                if (file_exists($imagePath)) {
                    ...
                    $value = '<!-- Gambar ' . e($key) . ' tidak ditemukan (' . e($value) . ') -->';
                }
            } else {
                // Teks biasa → escape karakter khusus
                $value = e($value);
            }
...
if (!function_exists('render_svg_to_file')) {
    function render_svg_to_file($templateName, $data = [], $templatePath = 'img/template/kartu_test', $subFolder = 'perangkat-test')
    {
        // 📄 Ambil template SVG
        $svgPath = public_path("$templatePath/$templateName.svg");
        if (!file_exists($svgPath)) {
            throw new Exception("Template SVG tidak ditemukan: $svgPath");
        }
        ...
        foreach ($data as $key => $value) {
            if (is_string($value) && file_exists($value)) {
                // kalau berupa file (logo/ttd)
                $ext = pathinfo($value, PATHINFO_EXTENSION);
                $mime = "image/" . ($ext === "jpg" ? "jpeg" : $ext);
                $base64 = "data:$mime;base64," . base64_encode(file_get_contents($value));
                $svgContent = str_replace("{{{$key}}}", $base64, $svgContent);
            } else {
                $svgContent = str_replace("{{{$key}}}", $value, $svgContent);
            }
        }

        // 📂 Folder: temp/{folderDinamis}
        $outputFolder = public_path("temp/{$subFolder}");
        ...
        // 🖼️ Nama file
        $fileName = Str::slug($data['nama_siswa'] ?? uniqid()) . '.svg';
        $outputFile = $outputFolder . '/' . $fileName;

        file_put_contents($outputFile, $svgContent);

        // 🔗 Return URL (bisa dipakai di <img src>)
        return asset("temp/{$subFolder}/{$fileName}");
    }
}

function render_svg_dynamic($templateName, $data, $templatePath, $subFolder)
{
    $templateFile = public_path("{$templatePath}/{$templateName}.svg");
    if (!File::exists($templateFile)) {
        throw new Exception("Template SVG tidak ditemukan: {$templateFile}");
    }

    $svgContent = File::get($templateFile);

    foreach ($data as $key => $value) {
        // 🔄 Kalau value adalah path gambar, ubah jadi base64 image
        if (is_string($value) && File::exists($value)) {
            $mime = mime_content_type($value);
            $base64 = base64_encode(File::get($value));
    ...
    // 🖼️ Nama file
    $fileName = Str::slug($data['nis'] ?? uniqid());
    $outputFile = "{$outputFolder}/{$fileName}.svg";

    File::put($outputFile, $svgContent);

    return asset("temp/{$subFolder}/{$fileName}.svg");
}
