...
if (!function_exists('isAwalBulanAktif')) {
    /**
     * Mengecek apakah hari ini waktu yang tepat untuk menjalankan tugas awal bulan.
     * - True jika hari ini tanggal 2 dan bukan Minggu
     * - True jika hari ini tanggal 3 dan kemarin (tgl 2) adalah Minggu
     */
    function isAwalBulanAktif(): bool
    {
        $now = Carbon::now();

        // Hari normal tanggal 2 dan bukan Minggu
        if ($now->day === 2 && !$now->isSunday()) {
            return true;
        }

        // Kalau tanggal 2 jatuh Minggu → geser ke tanggal 3
...
if (!function_exists('isAkhirBulanAktif')) {
    /**
     * Mengecek apakah hari ini saat yang tepat menjalankan rekap bulanan.
     * - True jika: hari terakhir bulan (bukan Minggu)
     * - True jika: hari ini H-1 karena besok Minggu
     * - False di hari lain
     */
    function isAkhirBulanAktif(): bool
    {
        $now = Carbon::now();
        $lastDayOfMonth = $now->copy()->endOfMonth();

        // Kondisi 1: Hari ini = tanggal terakhir bulan & bukan Minggu
        if ($now->isSameDay($lastDayOfMonth) && !$now->isSunday()) {
            return true;
        }

        // Kondisi 2: Hari terakhir jatuh Minggu → jalankan H-1
...
if (!function_exists('angka_romawi')) {
    /**
     * Konversi angka (1–100) ke angka Romawi
     *
     * @param int $angka
     * @return string
     */
