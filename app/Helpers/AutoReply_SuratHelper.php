    ...
    function SuratKode($NoRequest, $message)
    {
        ...
        switch ($Kode) {
            ...
            case 'Surat Home Visit': // Surat Home Visit/Surat/KodeGuru/Jabatan/nis/keperluan/+Hari/Waktu
                // Surat Home Visit/Surat/AS/Jabatan/250001/keperluan/1/09:30
            ...
            case 'Surat SPPD Custom':
                ...
                // Contoh 'Surat SPPD Custom / Surat / 2501:SPPD / 2 / AS:IZ / Kepala Madrasah:Guru / SMP Cipta IT / 2 / 2 / Motor / Sosialisasi aplikasi siakad';
            ...
            case 'Home Visit Custom':
                ...
                $keyMessage = 'Case / Surat / nomor_surat / tanggal_surat / kode_guru / jabatan_guru / tanggal_kunjungan / jam_kunjungan / tempat_kunjungan / tujuan_kunjungan / nis';
                // $message    = 'Home Visit Custom / Surat/2025/2/AS:DR/Guru BK:Wali Kelas/2/08:30/Rumah Siswa/Membicarakan Keaktifan Siswa/250001';
                // Contoh = 'Home Visit Custom / Surat / 235 / 2 / AS:DR / Guru BK:Wali Kelas / 2 / 08:30 / Rumah Siswa / Membicarakan kehadiran siswa / 250001 / Ya#No'; // Case / Guru / Kode_guru / ya#no
                // Contoh = 'Home Visit Custom/Surat/235/2/AS:DT/Guru BK:Wali Kelas/2/08:30/Rumah Siswa/Membicarakan kehadiran siswa/250001
                $keyMessage = preg_replace('/\s*\/\s*/', '/', trim($keyMessage));
                $message    = preg_replace('/\s*\/\s*/', '/', trim($message));
                ...
                $kodeGuru = array_map('trim', explode(':', $data['kode_guru']));
                $dataGuru = Detailguru::whereIn('kode_guru', $kodeGuru)->get();
                $namaGuru = implode("\n", array_map(
                    fn($i, $g) => ($i + 1) . ". " . $g->nama_guru,
                    array_keys($dataGuru->toArray()),
                    $dataGuru->all()
                ));

                // Jabatan
                // $jabatan_guru = explode(':', $data['jabatan_guru']);
                $jabatan_guru = array_map('trim', explode(':', $data['jabatan_guru']));
                $Listjabatan = '';
                $totalJabatan = count($jabatan_guru);

                foreach ($jabatan_guru as $index => $jabatan) {
                    $Listjabatan .= ($index + 1) . ". " . $jabatan;
                    if ($index < $totalJabatan - 1) {
                        $Listjabatan .= "\n";
                    }
                }
                ...
                $data['tahun'] = date('Y');
                $data['nama_ortu'] = 'Bapak ' . $datasiswa['ayah_nama'] . ' / Ibu ' . $datasiswa['ibu_nama'];
                //MTs Imut
