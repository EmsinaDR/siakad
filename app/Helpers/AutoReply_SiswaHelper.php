    ...
    function Auto_reply_SiswaKode($Siswa, $NoRequest, $message)
    {
        ...
        switch (ucfirst($Kode)) {
            case 'Data Siswa':
                ...
                // Pastikan file fallback benar-benar ada
                if (!file_exists($sourcePath)) {
                    // Kalau bahkan fallback pun gak ada, bikin notifikasi error aman
                    Log::warning("Foto siswa tidak ditemukan, fallback gagal: {$sourcePath}");
                    $sourcePath = public_path('img/default/blanko-foto.png');
                }

                // Pindahkan file ke folder WhatsApp sementara (untuk dikirim via sendMedia)
                ...
                $kirim = \App\Models\Whatsapp\WhatsApp::sendMedia($sessions, $NoRequest, $pesan, $targetPath);
                break;
            case 'Tabungan':
