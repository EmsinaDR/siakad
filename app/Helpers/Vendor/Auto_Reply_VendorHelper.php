    ...
    function Auto_Reply_VendorHelper($Kode, $NoRequest, $message, $sessions)
    {
        ...
        switch ($Kode) {
            ...
            case 'Fitur Aplikasi':
                ...
                break;
            case 'Teruskan':
                // Teruskan/Vendor/Guru:Siswa:Wali:Wali Kelas/Judul/isipesan/
                $pesan = explode('/', $message); // Memecah Array yang yang telah diterima, lanjut fungsi loop fetch
                //$ 1 : Yang akan Diganti
                //$ 2 : Yang akan Menggantikan
                //$ 3 : sumber kata / kalimat / variabel;
                $Judul = $pesan[3];
                $isipesan = str_replace('#', '/', $pesan[4]);
                /*
                ...
                */
                if ($pesan[2] === 'Guru') {
                    if (!config('whatsappSession.WhatsappDev')) {
                        ...
                        $Gurus = Detailguru::whereNotIn('id', [1, 2, 3])->limit(2)->get();
                    }
                    foreach ($Gurus as $guru):
                        ...
                        if (!config('whatsappSession.WhatsappDev')) {
                            ...
                            $NoTujuan = '6282328273372';
                        }
                        $result = \App\Models\Whatsapp\WhatsApp::sendMessage($sessions, $NoTujuan, format_pesan($Judul, $isipesan));
                    endforeach;
            ...
            case 'Aktivasi Modul':
                ...
                break;
            case 'Generate Kartu':
                // Generate Kartu / Vendor / nis / pembayaran:nisn:karpel:guru:absen:cocard
                $pesan = explode('/', $message);
                $dokumen = $pesan[3];
                // $DataNis
                if ($dokumen === 'karpel') {
                    $kodeKarpel = 11;
                    $folder = 'img/template/' . $dokumen;
                ...
                } elseif ($dokumen === 'pembayaran') {
                } elseif ($dokumen === 'guru') {
                } elseif ($dokumen === 'absen') {
                } elseif ($dokumen === 'cocard') {
                } else {
                }
                break;
            ...
            case 'Insert Siswa':
                ...
                try {
                    ...
                    $tapel = tapel();
                    $kelas = Ekelas::where('kelas', $parts[2])->first();

                    $siswa = new Detailsiswa();

                    // Isi data dari pesan
                    ArrayUpdateDataWa($parts, $siswa, 3);

                    // Tambahan info dari relasi
                    $siswa->kelas_id   = $kelas->id;
                    $siswa->tingkat_id = $kelas->tingkat_id;

                    // Buat user akun login
                    $user = new User();
                    $user->email       = $siswa->email;
                    $user->name        = ucwords($siswa->nama_siswa);
                    $user->tingkat_id  = $siswa->tingkat_id;
                    $user->posisi      = 'siswa';
                    $user->aktiv       = 1;
                    $user->password    = Hash::make($siswa->password);
                    $user->save();
                    ...
                    $allowed = Schema::getColumnListing('detailsiswas');
                    foreach (array_keys($siswa->getAttributes()) as $attr) {
                    ...
                    $PesanKirim = "✅ Data siswa berhasil ditambahkan:\n" .
                        json_encode($siswa, JSON_PRETTY_PRINT);
                    \App\Models\Whatsapp\WhatsApp::sendMessage($sessions, $NoRequest, $PesanKirim);

                    return false;
                } catch (\Exception $e) {
                    DB::rollBack();
                    $errorMsg = "❌ Gagal menyimpan data siswa:\n" . $e->getMessage();
                    \App\Models\Whatsapp\WhatsApp::sendMessage($sessions, $NoRequest, $errorMsg);
                    return false;
                }
                break;
            ...
            // $isiPesan =
            //     "✅ Data *baru* telah ditambahkan, harap periksa kembali:\n\n" .
            //     "{$dataUpdate}\n";

            // $pesanKiriman = format_pesan('Insert Data Siswa', $isiPesan);
            // $result = \App\Models\Whatsapp\WhatsApp::sendMessage($sessions, $NoRequest, $pesanKiriman);
            ...
            case 'Data SMP':
                // Data SMP / Vendor / Banjarharjo
                $pesan = explode("/", $message);
                if (isset($pesan[2])) {
                    $kecamtan = ucfirst(strtolower($pesan[2]));
                    $dataSmp = DataSekolahSMP::where('kecamatan', $kecamtan)->get();
                    $isi = $dataSmp->pluck('nama_sekolah')->filter()->implode("\n"); // ambil nama_sekolah aja
                    $total_siswa = 0; // 🧮 inisialisasi dulu sebelum foreach
                    $isi = "";
                    foreach ($dataSmp as $sekolah):
                        $jumlah_siswa = (int)$sekolah->siswa_l + (int)$sekolah->siswa_p;
                        $isi .= "*- " . $sekolah->nama_sekolah . "* - {$jumlah_siswa} Siswa\n";
                        $isi .= "Link :\n" . $sekolah->link_reference . "\n";
                        $isi .= "Kepala :\n" . $sekolah->nama_kepala . "\n";
                        $isi .= "Operator :\n" . $sekolah->nama_operator . "\n";
                        $isi .= "\n" . str_repeat("─", 25) . "\n";
                        $total_siswa += $jumlah_siswa;
                    endforeach;
                    $isiPesan =
                        "Kecamatan {$kecamtan}\n" .
                        "Total Sekolah : {$dataSmp->count()} Sekolah\n" .
                        "Total Siswa : {$total_siswa} Siswa\n" .
                        "\n" . str_repeat("─", 25) . "\n" .
                        "{$isi}" .
                        " \n ";
                } else {
                    $dataSmp = DataSekolahSMP::get();
                    $isi = $dataSmp->pluck('nama_sekolah')->filter()->implode("\n"); // ambil nama_sekolah aja
                ...
                break;
            case 'Command':
                // Command / Vendor/ namacommand (signatur)
                // Command / Vendor/ siakad:cobabuat
                $pesan = explode("/", $message);
                $output = run_artisan($pesan[2]);
                $isiPesan =
                    "Command {$pesan[2]} telah dijalankan\n" .
                    " \n ";
                $pesanKiriman = format_pesan('Data Link Website Saat Ini', $isiPesan);
                $result = \App\Models\Whatsapp\WhatsApp::sendMessage($sessions, $NoRequest, $pesanKiriman);
                break;
            case 'Guru Piket':
                // Format pesan: Guru Piket/Vendor/senin/AS:AN
                // Format pesan: Guru Piket/Vendor/senin/JM:SA
                // Format pesan: Guru Piket/Vendor/selasa/DT:NQ
                // Format pesan: Guru Piket/Vendor/rabu/ER:UN
                // Format pesan: Guru Piket/Vendor/kamis/MA:CY
                // Format pesan: Guru Piket/Vendor/jum'at/IM:SZ
                // Format pesan: Guru Piket/Vendor/sabtu/IZ
                $pesan = explode("/", $message);
                $tapel = tapel();
                $ArrayKodeGuru = explode(':', $pesan[3]);
                $pesanJson = json_encode($ArrayKodeGuru);

                // Normalisasi nama hari (huruf kecil semua)
                $hari = strtolower(trim($pesan[2]));

                foreach ($ArrayKodeGuru as $kodeGuru) {
                    $kodeGuru = trim($kodeGuru);
                    $guruData = Detailguru::where('kode_guru', $kodeGuru)->first();

                    if ($guruData) {
                        GuruPiket::updateOrCreate(
                            [
                                'tapel_id' => $tapel->id,
                                'detailguru_id' => $guruData->id,
                                'hari' => $hari, // sudah dinormalisasi
                            ],
                            [] // tidak ada field update tambahan
                        );
                    } else {
                        Log::warning("⚠️ Kode guru '{$kodeGuru}' tidak ditemukan saat input Guru Piket.");
                    }
                }

                $isiPesan = "✅ Jadwal piket hari *" . ucfirst($hari) . "* berhasil diperbarui tanpa duplikat.\n"
                    . "👥 Guru: {$pesanJson}";
                $pesanKiriman = format_pesan('Data Guru Piket', $isiPesan);
