...
if (!function_exists('bagiKelompokABX')) {
    /**
     * Bagi semua siswa aktif jadi dua kelompok (A & B),
     * lalu masukkan ke ruang-ruang (misal 15 A + 15 B per ruang),
     * dan buat pasangan di dalam masing-masing ruang.
     ...
     * @return array
     */
    function bagiKelompokABX(
        int $jumlahPerKelompokPerRuang = 15,
        string $nama_test = 'Ujian Tengah Semester',
        ...
        foreach ($semua as &$siswa) {
            ...
            if ($angkatan == '25') $tingkat = '7';
            elseif ($angkatan == '24') $tingkat = '8';
            elseif ($angkatan == '23') $tingkat = '9';
            else $tingkat = 'X';
        ...
        // 5️⃣ Gabungkan dan buat notest langsung di array hasil
        $jumlahRuang = max(count($ruangA), count($ruangB));
        $hasil = [];
        ...
        for ($i = 0; $i < $jumlahRuang; $i++) {
            ...
            for ($j = 0; $j < $maks; $j++) {
                ...
                if ($data1) {
                    $data1['notest'] = str_pad($data1['index_tingkat'], 3, '0', STR_PAD_LEFT)
                        . ' / R' . angka_romawi($i + 1)
                        . ' / ' . ($data1['kelas']['kelas'] ?? '-')
                        . ' / ' . $nama_test
                        . ' / ' . $bulan
                ...
                if ($data2) {
                    $data2['notest'] = str_pad($data2['index_tingkat'], 3, '0', STR_PAD_LEFT)
                        . ' / R' . angka_romawi($i + 1)
                        . ' / ' . ($data2['kelas']['kelas'] ?? '-')
                        . ' / ' . $nama_test
                        . ' / ' . $bulan
