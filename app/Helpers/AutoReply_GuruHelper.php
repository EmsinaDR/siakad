...
if (!function_exists('Auto_reply_Data_Guru')) {
    function Auto_reply_Data_Guru($Kode, $NoRequest, $message)
    {
        ...
        switch (ucfirst($Kode)) {
            ...
            case 'Kontak Kosong':
                ...
                $NoIbu = Detailsiswa::where('ibu_nohp', '')->get();
                $NoAyah = Detailsiswa::where('ayah_nohp', '')->get();
                $no_ibu = '';
                foreach ($NoIbu as $index => $ibu) {
                    $no = $index + 1;
                    $no_ibu .= "{$no}. {$ibu->nama_siswa} \n";
                }
                $DataIbu = "Berikut ini kontak *Ibu* yang kosong : \n{$no_ibu}";
                $no_ayah = '';
                foreach ($NoAyah as $index => $ayah) {
                    $no = $index + 1;
                    $no_ayah .= "{$no}. {$ayah->nama_siswa} \n";
                }
                $DataAyah = "Berikut ini kontak *Ayah* yang kosong : \n{$no_ayah}";
                $isiPesan =
                    "📬 *Berikut data kontak terkair no HP orang tua :*\n\n" .
                    "{$DataIbu}\n {$DataAyah}" .
                    "";
            ...
            case 'Link Absen':
                ...
                $guru  = config('app.name') . "absen-guru-ajax";
