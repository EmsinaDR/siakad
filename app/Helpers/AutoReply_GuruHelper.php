    ...
    function Auto_reply_Data_Guru($Kode, $NoRequest, $message, $sessions)
    {
        ...
        switch (ucfirst($Kode)) {
            case 'Data Guru': // Data Guru/Guru/Kode_Guru
                ...
                $filename = $pesan['kode_guru'] . '.png';
                CopyFileWa($filename, 'img/guru/foto');
                $filePath = base_path('whatsapp/uploads/' . $filename);
                $kirim = \App\Models\Whatsapp\WhatsApp::sendMedia($sessions, $NoRequest, $pesan['pesanKiriman'], $filePath); // Tambahkan movefiel ke folderwhatsapp karena hanya bisa kirim via folder whatsapp
                break;
            ...
            case 'Link Absen':
                ...
                $siswa = config('app.url') . "/absensi/absen-siswa-ajax";
                $guru  = config('app.url') . "/absensi/absen-guru-ajax";
