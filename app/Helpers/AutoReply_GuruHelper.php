    ...
    function Auto_reply_Data_Guru($Kode, $NoRequest, $message, $sessions)
    {
        ...
        switch (ucfirst($Kode)) {
            ...
            case 'Statistik Siswa':
                ...
                break;
            case 'Update Absen':
                //Update Absen / Guru / Kode Guru / arraynis / jenis absen / absen / keterangan
                // Update Absen / Guru / AS / 250051:250039:250039 / pulang#masuk / alfa:ijin:sakit / keterangan
                // Update Absen / Guru / AS / 250051:250039:250033/ pulang / alfa
                $pesan = explode('/', $message);
                $kodeguru = $pesan[2];
                $ArrayNis = explode(':', $pesan[3]);
                $jenis_absen = $pesan[4];
                ...
                if (isset($pesan[6])) {
                    ...
                    $keterangan = "diupdate oleh : $kodeguru";
                }
                $Guru = Detailguru::where('kode_guru', $kodeguru)->first();
                $siswas = Detailsiswa::whereIn('nis', $ArrayNis)->get();
                $ids = $siswas->pluck('id')
                ...
                $nama_siswa = "\n";
                foreach ($siswas as $siswa) {
                    $nama_siswa .= "- {$siswa->nama_siswa} (NIS: {$siswa->nis})\n";
                }
                $SiswaAbsen = Eabsen::whereIn('detailsiswa_id', $ids)
                    ->where('jenis_absen', $jenis_absen)
                    ->whereDate('waktu_absen', Carbon::now()->format('Y-m-d'))
                    ->update([
                        'absen' => $absen,
